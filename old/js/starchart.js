...
// 						StarChart.chart[i][j].exists = false;
  ...
  StarChart.initializeStars = function() {
    ...
    StarChart.skullSprite = new PIXI.Sprite(PIXI.loader.resources.skull.texture);
...
// 			StarChart.selectedStar.yWobble,StarChart.currentStar.x + StarChart.currentStar.xWobble,StarChart.currentStar.y + StarChart.currentStar.yWobble));
  ...
  StarChart.getStarInfoText = function() {
		...
		var level = Math.floor(((gameModel.p1.frontWeapon ? gameModel.p1.frontWeapon.level : 0) +
  ...
  StarChart.update = function(timeDiff) {
    ...
    if (gameModel.bossPosition) {
      ...
      StarChart.skullSprite.position.x = skullX + 30 * StarChart.zoom;
      ...
      StarChart.skullSprite.alpha = Math.min(StarChart.calcFade(skullX,skullY,bounds), 0.6);
      StarChart.skullSprite.scale.x = StarChart.skullSprite.scale.y = (0.2 + 0.05 * Math.sin(StarChart.skullSprite.scaleMod)) * scalingFactor;
    }
