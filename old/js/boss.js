...
Boss.bossActive = function() {
  return gameModel.bossPosition && gameModel.bossPosition.x == gameModel.targetSystem.x && gameModel.targetSystem.y == gameModel.bossPosition.y && Boss.health > 0;
}
...
Boss.randomLocation = function() {
  ...
  var currentLevel = Boss.levelsPerBoss * (gameModel.bossesDefeated + 1);
  var xLocation = 0;
  var yLocation = 0;
...
Boss.update = function(timeDiff) {
  ...
  if (Boss.collisionAllowed) {
    ...
    if (Boss.lastBullet >= 1 && Math.random() > 0.9 || Boss.bulletsLeft > 0) {
