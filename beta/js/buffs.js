Buffs = {
  ...
  update : function(timeDiff) {
    ...
    for (i = 0; i < Buffs.currentBuffs.length; i++) {
      ...
      Buffs.currentBuffs[i].container.anchor = {x:0.5,y:0.5};
			Buffs.currentBuffs[i].container.position = {x:(xPos + xPos * col) - Buffs.currentBuffs[i].container.width / 2,y:400 * scalingFactor - (row * scalingFactor * 50)};
  ...
  addSpreadShotBuff : function() {
    Buffs.newBuff(Buffs.buffNames.spreadShot, 10, "img/kunai.svg", Constants.itemColors.hyper);
  },
