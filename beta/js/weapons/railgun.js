...
RailGun.beamTrails = function(container) {
	this.spritePool = new SpritePool(Stars.stars.getTexture(), container);
};

RailGun.beamTrails.prototype.newPart = function(x, y) {
...
RailGun.beamTrails.prototype.update = function(timeDiff) {
...
RailGun.weaponLogic = function(weapon, container) {
	weapon.lastShot = 0;
	this.weapon = weapon;
	this.spritePool = new SpritePool(RailGun.generateTexture(), container);
	this.beamTrails = new RailGun.beamTrails(container);
};

RailGun.weaponLogic.prototype.fireShot = function(position, damageModifier) {
		...
		for (var j = 0; j < Enemies.activeShips.length; j ++) {
			if (!Enemies.activeShips[j].damagedByRailgun &&
				...
				if (Math.random() < this.weapon.empChance && !empFired) {
					EMP.newEmp(location.x, location.y, this.weapon.emp, sprite.tint, 300, this.weapon.emp);
					empFired = true;
				}
				damage *= 0.5;
			}
...
RailGun.weaponLogic.prototype.update = function(timeDiff) {
...
RailGun.weaponLogic.prototype.destroy = function() {
	this.spritePool.destroy();
};

RailGun.weaponLogic.prototype.readyToFire = function(isWeaponFiring, timeDiff, fireRateModifier) {
	if (isWeaponFiring) {
...
RailGun.weaponLogic.prototype.resize = function() {
